...
class ParametrizedClifford:
    ...
    def __init__(self, label: str, qbs: tuple[int] | tuple[int, int], param_dim: int):
        self.label = label
        self.qbs = qbs
...
class Parametrized1QClifford(ParametrizedClifford):
    ...
    def __init__(self, label: str, qb: int, param_dim: int):
...
class ParametrizedCliffordCircuit:
    ...
    def read(self):
        ...
        pgates_total_idc = [
            i for (i, gate) in enumerate(self.gates) if not gate.is_fixed()
        ]
        return [
            self.gates[pgates_total_idc[param_map(i)]].k
            for i in range(self.number_parametrized_gates())
        ]
    ...
    def assign(self, params: list[int]):
        ...
        assert num_param_gates == len(
            params
        ), f"Number of parameters given ({len(params)}) not consistent with number of parametrized gates ({num_param_gates})."
        ...
        if param_map_dict is None:
            ...
            def param_map(i):
                if i in param_map_dict:
                    ...
                    raise Exception(
                        f"circuit param idx {i} was not defined in custom map"
                    )
            ...
            def inv_param_map(i):
                if i in inv_param_map_dict:
                    ...
                    raise Exception(
                        f"internal pcirc param idx {i} was not defined in custom map"
                    )
        ...
        if meas_map_dict is None:
            ...
            def meas_map(i):
                if i in meas_map_dict:
                    ...
                    raise Exception(
                        f"measurement input {i} was not defined in custom map"
                    )
            ...
            def inv_meas_map(i):
                if i in inv_meas_map_dict:
                    ...
                    raise Exception(
                        f"measurement output {i} was not defined in custom map"
                    )
    ...
    def idc_param_2qb(self):
        ...
        return [
            ...
            if isinstance(gate, Parametrized2QClifford) and not gate.is_fixed()
        ]
